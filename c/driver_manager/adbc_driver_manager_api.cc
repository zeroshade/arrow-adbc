...
AdbcStatusCode AdbcDatabaseInit(struct AdbcDatabase* database, struct AdbcError* error) {
  ...
  if (args->init_func) {
    status =
        AdbcLoadDriverFromInitFunc(args->init_func, ADBC_VERSION_1_1_0,
                                   database->private_driver, error);
  } else {
    const char* entrypoint = args->entrypoint.empty() ? nullptr : args->entrypoint.c_str();
    const char* additional_paths = args->additional_search_path_list.empty()
                                       ? nullptr
                                       : args->additional_search_path_list.c_str();

    status =
        AdbcFindLoadDriver(args->driver.c_str(), entrypoint, ADBC_VERSION_1_1_0,
                          args->load_flags, additional_paths, database->private_driver, error);
  }
  ...
  for (const auto& [key, value] : args->options) {
    status =
        database->private_driver->DatabaseSetOption(database, key.c_str(), value.c_str(), error);
    if (status != ADBC_STATUS_OK) {
  ...
  for (const auto& [key, value] : args->bytes_options) {
    status = database->private_driver->DatabaseSetOptionBytes(
        database, key.c_str(), reinterpret_cast<const uint8_t*>(value.data()), value.size(),
        error);
    if (status != ADBC_STATUS_OK) {
      database->private_driver->DatabaseRelease(database, error);
      ReleaseDriver(database->private_driver, nullptr);
  ...
  for (const auto& [key, value] : args->int_options) {
    status =
        database->private_driver->DatabaseSetOptionInt(database, key.c_str(), value, error);
    if (status != ADBC_STATUS_OK) {
      database->private_driver->DatabaseRelease(database, error);
      ReleaseDriver(database->private_driver, nullptr);
      delete database->private_driver;
  ...
  for (const auto& [key, value] : args->double_options) {
    status =
        database->private_driver->DatabaseSetOptionDouble(database, key.c_str(), value, error);
    if (status != ADBC_STATUS_OK) {
      database->private_driver->DatabaseRelease(database, error);
      ReleaseDriver(database->private_driver, nullptr);
      delete database->private_driver;
...
AdbcStatusCode AdbcConnectionGetStatisticNames(struct AdbcConnection* connection,
  ...
  INIT_ERROR(error, connection);
  WRAP_STREAM(connection->private_driver->ConnectionGetStatisticNames(connection, out, error),
              out, connection);
}
...
AdbcStatusCode AdbcConnectionInit(struct AdbcConnection* connection,
  ...
  for (const auto& [key, value] : args->double_options) {
    status = connection->private_driver->ConnectionSetOptionDouble(connection, key.c_str(),
                                                                   value, error);
    if (status != ADBC_STATUS_OK) {
      connection->private_driver->ConnectionRelease(connection, error);
      connection->private_driver = nullptr;
